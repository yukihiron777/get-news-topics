...
## 概要
- 取得済みの JSON (`data/nikkei/YYYY-MM-DD.json` または `data/nikkei/latest.json`) をもとに、自動で Markdown 記事を生成し、GitHub Dispatch で公開リポジトリに送信する仕組み。
- `scripts/batch-generate-from-daily.ts` が記事ファイルと `data/nikkei/article-status.json` を更新、`scripts/dispatch-article.ts` が GitHub API を呼び出す。
...
- `npm run process:daily -- latest` : `data/nikkei/latest.json` を対象とする。
...
## 再送防止ロジック
- `data/nikkei/article-status.json` に slug ごとの `status` / `queuedAt` / `dispatchedAt` を記録し、`dispatch-article.ts` は `drafted` か `queued` の記事のみを対象にする。
