...
if [ $# -lt 1 ]; then
  echo "使い方: npm run process:daily -- 2026-01-04 | data/nikkei/2026-01-04.json | latest" >&2
  exit 1
fi

INPUT="$1"
if [[ "$INPUT" == latest ]]; then
  DAILY_FILE="data/nikkei/latest.json"
elif [[ "$INPUT" == *.json ]]; then
  DAILY_FILE="$INPUT"
else
  DAILY_FILE="data/nikkei/$INPUT.json"
fi
...
SLUGS=$(jq -r --arg key "$DATE_KEY" '.[$key][] | select(.status=="drafted") | .slug' data/nikkei/article-status.json)
if [ -z "$SLUGS" ]; then
